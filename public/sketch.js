...
function saveApiKeyFromInput() {
  ...
  refreshAuthPanel('Personal API key saved for this tab only.');
}
...
function clearStoredApiKey() {
  ...
  authPromptForced = true;
  refreshAuthPanel('Saved API key cleared from this tab.');
}
...
function refreshAuthPanel(message) {
  ...
  if (modeCopy) {
    if (appConfig.authMode === 'server-key') {
      ...
      modeCopy.html('This publishable build does not include any shared secret. To generate AI output, paste your own Google AI Studio API key below. It will be kept only in memory for the current tab.');
    }
  }
