...
function getRequestApiKey(headers = {}) {
  ...
  const headerKey = headers['x-google-ai-studio-key'] || headers['X-Google-AI-Studio-Key'];
...
function getEffectiveApiKey(headers = {}) {
  return process.env.GOOGLE_API_KEY || getRequestApiKey(headers);
}
...
async function interpretAndGenerate({ headers = {}, musicSummary, interpretation }) {
  ...
  if (!apiKey) {
    const error = new Error('No Google AI Studio API key is available. Add GOOGLE_API_KEY on the server or provide your own key in the app.');
    error.status = 401;
    throw error;
  }
